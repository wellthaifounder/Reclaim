...
**Issue:** `supabase db push` refused to apply _any_ migration — including a new one needed to ship a feature — failing with `Remote migration versions not found in local migrations directory: 20260109 20260125 20260412 20260424`. Nothing was actually wrong with the database. Those four files carried an 8-digit version prefix (`20260109_add_wellbie_attachments.sql`) _and_ had a 14-digit sibling on the same date (`20260109010000_extend_receipt_ocr_for_metadata.sql`). The CLI pairs local files against the remote history table with a merge join over sorted versions; an 8-digit version that is a string prefix of a 14-digit version on the same date sorts inconsistently between the two sides, so the join drops both and each appears unmatched. `--include-all` does not help — the check runs first. Fourteen other 8-digit files matched fine, because none of them shares a date with a 14-digit file.
